...
def get_passthrough_cols(
    ...
    for col in schema:
        ...
        if name.startswith("n_distinct_"):
            excluded_qc.append(name)
            continue
        if is_array_type_fn(col["type"]):
            continue
        passthrough.append(name)

    if excluded_qc:
        logger.info(
            "Excluding QC/lineage columns from output: %s", excluded_qc
        )

    logger.debug("Passthrough columns: %s", passthrough)
    return passthrough